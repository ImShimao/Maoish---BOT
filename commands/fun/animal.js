...
module.exports = {
    ...
    async execute(interactionOrMessage, args) {
        ...
        const translationMap = {
            'blobfish': 'blobfish', 'axolotl': 'axolotl', 'loutre': 'otter', 'paresseux': 'sloth',
            'singe': 'monkey', 'gorille': 'gorilla', 'chimpanzé': 'chimpanzee', 'cheval': 'horse',
            'licorne': 'unicorn', 'requin': 'shark', 'dauphin': 'dolphin', 'pieuvre': 'octopus',
            'méduse': 'jellyfish', 'serpent': 'snake', 'cobra': 'cobra', 'lézard': 'lizard',
            'caméléon': 'chameleon', 'tortue': 'turtle', 'grenouille': 'frog', 'crapaud': 'toad',
            'aigle': 'eagle', 'faucon': 'falcon', 'hibou': 'owl', 'perroquet': 'parrot',
            'flamant': 'flamingo', 'pingouin': 'penguin', 'lion': 'lion', 'tigre': 'tiger',
            'guépard': 'cheetah', 'léopard': 'leopard', 'panthère': 'panther', 'loup': 'wolf',
            'ours': 'bear', 'polaire': 'polar bear', 'grizzly': 'grizzly bear', 'hérisson': 'hedgehog',
            'écureuil': 'squirrel', 'castor': 'beaver', 'lapin': 'rabbit', 'lièvre': 'hare',
            'hamster': 'hamster', 'cochon d\'inde': 'guinea pig', 'souris': 'mouse', 'rat': 'rat',
            'cerf': 'deer', 'élan': 'moose', 'chameau': 'camel', 'dromadaire': 'dromedary',
            'lama': 'llama', 'alpaga': 'alpaca', 'girafe': 'giraffe', 'zèbre': 'zebra',
            'rhinocéros': 'rhinoceros', 'hippo': 'hippopotamus', 'éléphant': 'elephant',
            'vache': 'cow', 'taureau': 'bull', 'mouton': 'sheep', 'chèvre': 'goat',
            'cochon': 'pig', 'poule': 'chicken', 'coq': 'rooster', 'poussin': 'chick',
            'dindon': 'turkey', 'oie': 'goose', 'abeille': 'bee', 'papillon': 'butterfly',
            'araignée': 'spider', 'scorpion': 'scorpion', 'ornithorynque': 'platypus',
            'tatou': 'armadillo', 'tapir': 'tapir', 'suricate': 'meerkat', 'lémurien': 'lemur'
        };
        ...
        try {
            ...
            // --- ENVOI DE L'EMBED AVEC USINE ---
            const embed = embeds.info(interactionOrMessage, `${finalEmoji} Voici un(e) ${targetAnimal.charAt(0).toUpperCase() + targetAnimal.slice(1)} !`, null)
                .setColor(finalColor) // On force la couleur
                .setImage(imageUrl)
            ...
            await replyFunc({ 
                embeds: [embeds.error(interactionOrMessage, `Oups ! Impossible de trouver un **${targetAnimal}**. (Erreur API)`)] 
            });
        }
    }
};